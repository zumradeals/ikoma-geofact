...
class WialonSyncPage extends Page
{
    ...
    /**
     * Déclenche la synchronisation Wialon immédiatement sans attendre le cron.
     * Utile quand le scheduler cPanel est arrêté ou pour un rechargement manuel.
     */
    public function triggerSync(): void
    {
        try {
            app()->call([new WialonSyncJob(), 'handle']);

            Notification::make()
                ->title('Synchronisation lancée')
                ->body('Le pipeline Wialon a été exécuté. Rechargez la page pour voir le résultat.')
                ->success()
                ->send();
        } catch (\Throwable $e) {
            Notification::make()
